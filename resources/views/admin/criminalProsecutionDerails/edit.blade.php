...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.criminalProsecutionDerail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.criminal-prosecution-derails.update", [$criminalProsecutionDerail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('criminal_prosecution') ? 'is-invalid' : '' }}" name="criminal_prosecution_id" id="criminal_prosecution_id" required>
                    @foreach($criminal_prosecutions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('criminal_prosecution_id') ? old('criminal_prosecution_id') : $criminalProsecutionDerail->criminal_prosecution->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('govt_order_no') ? 'is-invalid' : '' }}" type="text" name="govt_order_no" id="govt_order_no" value="{{ old('govt_order_no', $criminalProsecutionDerail->govt_order_no) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('govt_order_date') ? 'is-invalid' : '' }}" type="text" name="govt_order_date" id="govt_order_date" value="{{ old('govt_order_date', $criminalProsecutionDerail->govt_order_date) }}">
