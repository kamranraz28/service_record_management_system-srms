...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.diciplinaryAction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.diciplinary-actions.update", [$diciplinaryAction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="govt_order_no">{{ trans('cruds.diciplinaryAction.fields.govt_order_no') }}</label>
                <input class="form-control {{ $errors->has('govt_order_no') ? 'is-invalid' : '' }}" type="text" name="govt_order_no" id="govt_order_no" value="{{ old('govt_order_no', $diciplinaryAction->govt_order_no) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('govt_order_date') ? 'is-invalid' : '' }}" type="text" name="govt_order_date" id="govt_order_date" value="{{ old('govt_order_date', $diciplinaryAction->govt_order_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('diciplinary_action') ? 'is-invalid' : '' }}" name="diciplinary_action_id" id="diciplinary_action_id" required>
                    @foreach($diciplinary_actions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('diciplinary_action_id') ? old('diciplinary_action_id') : $diciplinaryAction->diciplinary_action->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
