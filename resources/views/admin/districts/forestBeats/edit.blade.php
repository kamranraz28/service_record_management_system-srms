...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.forestBeat.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.forest-beats.update', [$forestBeat->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-select select2 {{ $errors->has('forest_range') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($forest_ranges as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('forest_range_id') ? old('forest_range_id') : $forestBeat->forest_range->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_bn') ? 'is-invalid' : '' }}" type="text"
                        name="name_bn" id="name_bn" value="{{ old('name_bn', $forestBeat->name_bn) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text"
                        name="name_en" id="name_en" value="{{ old('name_en', $forestBeat->name_en) }}" required>
