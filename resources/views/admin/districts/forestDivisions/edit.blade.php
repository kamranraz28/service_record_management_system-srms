...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.forestDivision.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.forest-divisions.update', [$forestDivision->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-select select2 {{ $errors->has('forest_state') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($forest_states as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('forest_state_id') ? old('forest_state_id') : $forestDivision->forest_state->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_bn') ? 'is-invalid' : '' }}" type="text"
                        name="name_bn" id="name_bn" value="{{ old('name_bn', $forestDivision->name_bn) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text"
                        name="name_en" id="name_en" value="{{ old('name_en', $forestDivision->name_en) }}" required>
