...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.examBoard.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.exam-boards.update', [$examBoard->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-select select2 {{ $errors->has('examination') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($examinations as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('examination_id') ? old('examination_id') : $examBoard->examination->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_bn') ? 'is-invalid' : '' }}" type="text"
                        name="name_bn" id="name_bn" value="{{ old('name_bn', $examBoard->name_bn) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text"
                        name="name_en" id="name_en" value="{{ old('name_en', $examBoard->name_en) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text"
                        name="description" id="description" value="{{ old('description', $examBoard->description) }}">
