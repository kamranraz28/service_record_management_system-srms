...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.projectRevenuelone.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.project-revenuelones.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-select select2 {{ $errors->has('project_revenue') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($project_revenues as $id => $entry)
                            <option value="{{ $id }}" {{ old('project_revenue_id') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
