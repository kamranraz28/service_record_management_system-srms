            ...
            <form method="POST" action="{{ route('admin.results.update', [$result->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_bn') ? 'is-invalid' : '' }}" type="text"
                        name="name_bn" id="name_bn" value="{{ old('name_bn', $result->name_bn) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text"
                        name="name_en" id="name_en" value="{{ old('name_en', $result->name_en) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-select select2 {{ $errors->has('resultgroup') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($resultgroups as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('resultgroup_id') ? old('resultgroup_id') : $result->resultgroup->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
