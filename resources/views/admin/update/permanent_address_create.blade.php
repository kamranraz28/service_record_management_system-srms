...
@section('content')
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    <hr>
                        <h4>{{ trans('cruds.addressdetaile.title_new') }}</h4>
                        <br>
                        <form method="POST"
                            action="{{ route('admin.address.permanentAddressStore') }}"
    ...
    <select class="form-control select2 {{ $errors->has('thana_upazila') ? 'is-invalid' : '' }}" name="thana_upazila_id" id="thana_upazila_id" required>
        @foreach ($thana as $thana_item)
            <option value="{{ $thana_item->id }}"
                {{ (old('thana_upazila_id') ? old('thana_upazila_id') : $employee->thana_upazila_id ?? '') == $thana_item->id ? 'selected' : '' }}>
