    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
			...
			<div class="form-group">
                <label for="forest_circle_id">অঞ্চল</label>
                <select class="form-control select2 {{ $errors->has('forest_circle_id') ? 'is-invalid' : '' }}" name="forest_circle_id" id="forest_circle_id">
                    @foreach($circles as $id => $entry)
                        <option value="{{ $id }}" {{ old('forest_circle_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
			...
			<div class="form-group">
                <label for="forest_division_id">বন বিভাগ</label>
                <select class="form-control select2 {{ $errors->has('forest_division_id') ? 'is-invalid' : '' }}" name="forest_division_id" id="forest_division_id">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('designations') ? 'is-invalid' : '' }}" name="designations_id" id="designations_id">
                    @foreach($designations as $id => $entry)
                        <option value="{{ $id }}" {{ old('designations_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
